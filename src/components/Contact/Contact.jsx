...
const InputField = ({ label, type = 'text', name, value, onChange, error, placeholder, rows }) => (
  <div>
    <label className="text-white/60 text-xs font-mono uppercase tracking-wider mb-2 block">{label}</label>
    {rows ? (
      <textarea
        name={name}
        value={value}
        ...
        style={{ border: error ? '1px solid rgba(255,80,80,0.5)' : '1px solid rgba(212,175,55,0.1)' }}
      />
    ) : (
      <input
        type={type}
    ...
    <section id="contact" className="relative py-10 bg-dark overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-10">
        ...
        >
          ...
          <h2 className="section-heading">Get In <span className="gold-text">Touch</span></h2>
        ...
        <div className="grid lg:grid-cols-5 gap-12">
          ...
          >
            <h3 className="font-display text-2xl font-bold text-white mb-6">
              Let's build something<br /><span className="gold-text">amazing together</span>
            </h3>
            ...
            {contactInfo.map(({ icon: Icon, label, value, href }) => (
              ...
              >
                ...
                <div>
                  ...
                  {href ? (
                    <a href={href} className="text-white hover:text-gold transition-colors text-sm font-medium">{value}</a>
                  ) : (
            ...
            {/* Status */}
            <div className="glass rounded-xl p-4 mt-8" style={{ border: '1px solid rgba(212,175,55,0.15)' }}>
              <div className="flex items-center gap-3">
                <div className="w-2.5 h-2.5 rounded-full bg-green-400 animate-pulse" />
                <div>
                  <p className="text-white text-sm font-medium">Currently Available</p>
                  <p className="text-white/40 text-xs">Open to full-time & freelance</p>
                </div>
              </div>
            </div>
          </motion.div>
            ...
            <AnimatePresence mode="wait">
              {submitted ? (
                <motion.div
                  ...
                  style={{ border: '1px solid rgba(212,175,55,0.2)', minHeight: '400px' }}
                >
                  <motion.div
                    initial={{ scale: 0 }}
                    animate={{ scale: 1 }}
                  ...
                  <p className="text-white/50 mb-6">Thanks for reaching out. I'll get back to you within 24 hours.</p>
                ...
                >
                  <div className="grid sm:grid-cols-2 gap-5">
                    <InputField label="Full Name" name="name" value={form.name} onChange={handleChange} error={errors.name} placeholder="Sudheer Pal" />
                    <InputField label="Email Address" type="email" name="email" value={form.email} onChange={handleChange} error={errors.email} placeholder="you@example.com" />
                  </div>
                  <InputField label="Subject" name="subject" value={form.subject} onChange={handleChange} error={errors.subject} placeholder="Project Discussion" />
                  <InputField label="Message" name="message" value={form.message} onChange={handleChange} error={errors.message} placeholder="Tell me about your project..." rows={5} />
