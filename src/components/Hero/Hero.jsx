...
const floatingTech = [
  { label: 'React', angle: 0,   radius: 160, emoji: '⚛️' },
  { label: 'Node',  angle: 72,  radius: 150, emoji: '🟢' },
  { label: 'Mongo', angle: 144, radius: 165, emoji: '🍃' },
  { label: 'JS',    angle: 216, radius: 155, emoji: '⚡' },
  { label: 'CSS',   angle: 288, radius: 160, emoji: '🎨' },
]
...
const TypingText = () => {
  ...
  return (
    <span className="gold-text font-display">
      {text}
      <span className="inline-block w-[3px] h-[1em] bg-gold ml-1 align-middle" style={{ animation: 'blink 0.8s step-end infinite' }} />
    </span>
  )
}
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        {[...Array(5)].map((_, i) => (
          <motion.div key={i}
            ...
            style={{ width: `${80+i*40}px`, height: `${80+i*40}px`, left: `${10+i*15}%`, top: `${20+(i%3)*20}%` }}
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center min-h-[calc(100vh-120px)]">
          ...
          <div className="order-2 lg:order-1">
            ...
            >
              ...
              <span className="text-white/70 text-xs font-mono tracking-wider">Available for Work</span>
            </motion.div>
            ...
            >
              ...
              I build modern, scalable, responsive, and user-friendly web applications
            ...
            >
              <motion.a whileHover={{ scale:1.05, y:-2 }} whileTap={{ scale:0.95 }}
                href="/images/Sudheer Pal FullStack.pdf" download className="btn-primary flex items-center gap-2"
              >
                <span className="relative z-10 flex items-center gap-2"><Download size={16} />Download Resume</span>
              </motion.a>
              <motion.button whileHover={{ scale:1.05, y:-2 }} whileTap={{ scale:0.95 }}
                onClick={scrollToContact} className="btn-outline"
            ...
            <div className="relative w-72 h-72 sm:w-80 sm:h-80 lg:w-96 lg:h-96">
              ...
              {[0,1,2].map(i => (
                <motion.div key={i}
                  ...
                  style={{ margin: `${-20-i*20}px`, animation: `spin ${20+i*10}s linear infinite ${i%2===0?'':'reverse'}` }}
                />
              ))}
              ...
              {floatingTech.map((tech, i) => {
                ...
                return (
                  <motion.div key={tech.label}
                    className="absolute glass rounded-xl px-2 py-1.5 flex items-center gap-1.5 text-xs font-mono"
                    style={{ left:`calc(50% + ${x}px - 30px)`, top:`calc(50% + ${y}px - 16px)` }}
                    animate={{ y:[0,-10,0], opacity:[0.7,1,0.7] }}
                    transition={{ duration:3+i*0.5, repeat:Infinity, ease:'easeInOut', delay:i*0.4 }}
                  >
                    <span>{tech.emoji}</span>
                    <span className="text-white/70 hidden sm:inline">{tech.label}</span>
                  </motion.div>
                )
              })}
              ...
              <motion.div
                ...
                style={{
                  ...
                  boxShadow: '0 0 60px rgba(212,175,55,0.35), 0 0 120px rgba(212,175,55,0.12), inset 0 0 40px rgba(0,0,0,0.5)',
                }}
              >
                <img
                  src={profileImg}
                ...
                <div className="absolute inset-0 rounded-full pointer-events-none"
                  style={{ background: 'linear-gradient(180deg, rgba(212,175,55,0.05) 0%, rgba(0,0,0,0.35) 100%)' }}
                />
              </motion.div>
              ...
              <motion.div
                ...
                style={{ background: 'radial-gradient(circle at center, rgba(212,175,55,0.18) 0%, transparent 70%)' }}
              />
            </div>
          </motion.div>
        </div>
